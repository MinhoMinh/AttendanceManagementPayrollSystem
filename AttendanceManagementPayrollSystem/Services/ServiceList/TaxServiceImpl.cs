...
namespace AttendanceManagementPayrollSystem.Services.ServiceList
{
    public class TaxServiceImpl : TaxService
    {
        ...
        {
            var entity = await taxRepository.AddTaxAsync(dto);
        ...
        {
            return await taxRepository.GetActiveTaxDTO();
        }

        public async Task<List<TaxDTO>> GetAllTaxDTOs()
        {
            return await taxRepository.GetTaxDTOs();
        }

        public async Task<TaxGroupDTO> GetTaxGroupAsync()
        {
            return await taxRepository.GetTaxGroupAsync();
        }
        public async Task<bool> UpdateStatusAsync(int id, bool newStatus)
        {
            var oldTax = await taxRepository.GetActiveTax();
        ...
        {
            var entity = await taxRepository.GetByIdAsync(id);
